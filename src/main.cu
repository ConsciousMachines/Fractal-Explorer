...
int render_options(Camera& camera) 
{
...
int main()
{
    ...
    int interaction; // did we interact with ImGui this frame?
    // while (!glfwWindowShouldClose(window))
    while (camera.should_render)
    {
        // CUDA rendering
        if (camera.is_mapped_pbo == false)
        {
            checkCudaErrors(cudaGraphicsMapResources(1, &cuda_pbo_resource, 0));
            camera.is_mapped_pbo = true;
        }
        size_t num_bytes;
        checkCudaErrors(cudaGraphicsResourceGetMappedPointer((void **)&d_dst, &num_bytes, cuda_pbo_resource));
        // thread_spawner(d_dst, numSMs);
        camera.move();
        if (camera.is_mapped_pbo == true)
        {
            checkCudaErrors(cudaGraphicsUnmapResources(1, &cuda_pbo_resource, 0));
            camera.is_mapped_pbo = false;
        }
        ...
        // Rendering
        interaction = camera.render_options(camera);
        ImGui::Render();
        ...
        // take photograph
        if (interaction || camera.is_moving)
        {
            camera.take_photograph();
        }
